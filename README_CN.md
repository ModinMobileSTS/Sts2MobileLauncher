...
在 Android 上，full 兼容包会补回 PC 本体场景中缺失的联机表情按钮。进入多人大厅后（包括角色选择/准备界面和显示中的等待面板），游戏正式开始前也会保留该按钮；按住右下角悬浮按钮并拖动选择原版表情轮盘的一项，松手后通过本体已有的反应同步器发送。“附加设置 → 系统 → 显示联机表情按钮”可实时控制显示，默认开启。
