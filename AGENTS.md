...
- `GodotApp` 启动行为：
  ...
  - 可选**应用内**快捷面板（非系统悬浮窗，不需 `SYSTEM_ALERT_WINDOW`）：设置 `android_in_game_overlay_enabled`（默认关）后在 `GodotApp` 上叠可拖动、自动贴边且避开 system bars/display cutout 的“快捷”胶囊入口；点击打开从左侧滑入的无标题快捷抽屉，宽度约占可用屏幕 40%，左侧为竖向图标页签，右侧暗色空白区和系统 Back 都会先关闭抽屉，触控目标至少 48dp。提供打开附加设置、打开软键盘（与音量上键共用 `showSoftKeyboardForGame`）、重启游戏进程、存档快照、退出启动器、热设置子集、实时日志（优先 `godot.log` 其次 `sts2.log`）；日志页使用 `RecyclerView` 行列表复用，日志正文按等级着色，右侧提供顶部、底部、默认开启的自动贴底和筛选齿轮按钮，日志源/等级按钮组（`V/D/I/W/E`）与搜索框默认隐藏，点齿轮后显示且启用项背景高亮。从附加设置返回后会按当前开关拆除或重建入口。开发者工具 `android_dev_tools_enabled` 开启后显示专业化检查器：Runtime 页浏览 STS2/compat C# 根对象，Scene 页显示当前 Godot scene tree 并可像编辑器 inspector 一样查看 Node 属性，行内用紧凑 `RecyclerView`、类型徽标、名称/类型/值分色显示；顶部操作统一为等尺寸图标按钮且只在可返回时显示返回键；scene tree 默认全折叠，节点层级以背景色区分，点击卡片展开/折叠、点击右侧箭头进入 Node；Node/Godot 自定义对象属性以 `ToString()` 风格预览并可继续嵌套下钻。`android_dev_inspector_writable` 控制是否允许写入简单值和执行临时 GDScript（变量 `root` / `tree` / `node`），写入/脚本操作审计到 `<files>/logs/dev-tools.log`；不再提供独立 Node 方法调用入口。GodotApp 的 DeviceDefault theme 上创建 Material 对话框时必须用 `Theme.Sts2ExtraSettings` 的 `ContextThemeWrapper`，不可直接传 activity。检查器只由当前 full compat 的 `DevToolsHost` 提供，offline bootstrap 或旧完整包会显示可操作的不可用提示；Host 独立于其他 feature patch 启动，使用 `<files>/launcher/devtools/host.json` ready marker 及 protocol 2 的原子 `response-<uuid>.json`，Java 仍可读取旧 `response.json`，只读请求在 host 存活而超时时最多重试一次。
