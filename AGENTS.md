...
2. 按当前启动配置的 `compat_pack_id` 复制 `STS2Mobile.dll` 到 publish 目录；关闭兼容包开关时删除该 dll。
3. 复制当前启动配置兼容包的 `port_compat.pck` 到 `<files>/port_compat.pck`；无选择时使用 `android/assets/port_compat.pck` fallback（正常 launcher 启动会先阻止缺包场景）。
...
12. `TransitionMaterialPatches` 会复制 `NTransition` 使用的 fade/fight `ShaderMaterial`，避免关闭预加载时原版 missed-cache 清理 dispose 掉仍在 `FadeIn()` 中使用的共享 transition 材质，从而黑屏。
...
上述 MOD 初始化时序、本地 LAN patch 自动跳过大厅 MOD、预加载/tooltip 设置协议、shader 兼容排除卡面 `canvas_group_mask_blur`、快速重开 async 时序修复是 `compat/v0.103.2`、`compat/v0.106.1-beta` 与 `compat/v0.107.0-beta` 内置包都应保持的相同不变式；跨版本热修需同步到 `tools/android/stage-bundled-compat-packs.sh` 的 worktree 注入列表。详细流程见 `doc/runtime/compat-pack-loading-flow.md`。
