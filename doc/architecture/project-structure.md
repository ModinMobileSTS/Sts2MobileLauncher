...
- `SteamWorkshopActivity`：Steam 创意工坊页面，未登录时通过 Steam Community 公开 Workshop 页面匿名浏览塔2公开条目，已登录时优先复用 Steam 中心保存的 refresh token 走 Steam CM 查询并在失败时回落到公开浏览；默认开启“创意工坊兼容访问”，对 `steamcommunity.com` / `api.steampowered.com` 使用与 WorkshopAndroidDownloader 相同的 rmbgame 转发路径，以处理部分网络下原始 Steam Community 直连超时。下载条目通过 MOD 导入 staging 流程安装到当前 launch profile MOD 根，匿名状态会尝试公开/匿名下载路径，部分受限条目仍可能需要登录。下载后在 `<files>/workshop/library/index.json` 记录 `published_file_id`、远端更新时间、导入的 MOD ID 和安装摘要，用于后续更新检查。页面内包含创意工坊网页入口、已下载列表、手动检查更新、导入分组、兼容访问开关和 UGC 分块并发设置。
