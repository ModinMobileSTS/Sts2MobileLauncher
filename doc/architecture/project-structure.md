...
- `SteamDepotDirectoryDownloader` / `SteamCdnTransport`：SteamPipe manifest/chunk 传输层。上层在筛选 depot 时把已解析的 prepared manifest 直接交给目录下载器，避免正式下载前重复拉取 manifest。chunk 下载使用 1 / 2 / 4 个 worker、容量受限的结果通道和单 writer；在途压缩包与解压结果按 JVM 最大堆的约 `1/12` 计预算，并限制在 16–64 MiB。本体文件最终按 manifest SHA-1 严格校验，不能只凭 chunk Adler32 放行。Steam 目录返回 `use_as_proxy` 时，传输层显式尝试 proxy route，再回落 origin route，并遵守 `bypass_proxies_of_type`；depot auth token 只会发给 Steam 返回的 origin/proxy 路由，不会交给 Steam Community 图片/API 使用的 rmbgame 兼容访问。OkHttp 使用 coroutine-aware 异步 Call，取消下载会向下取消正在执行的网络请求；CDN token 按过期时间缓存，403 刷新按 host 合并并发请求。payload 下载/安装还用 `<files>/steam/downloads/locks/payload-download.lock` 做全局独占，避免 Activity 重建或不同 branch 任务同时进入 staging/import。
...
- `GodotApp`：真正的 Godot Activity，拼接 Godot 命令行，加载 imported PCK 或 bootstrap PCK，暴露 Java bridge 给 C#；干净退出回设置时写入云存档/本地快照自动处理 marker。Java 不再把 `fullscreen_render_size` 追加为 Godot `--resolution`；动态渲染分辨率由 compat 在根 renderer viewport 内处理。窗口焦点变化只更新 Java 侧生命周期状态，不再直接调用 `GodotLib.focusin/focusout`，避免与 Godot Activity 自身 pause/resume 焦点派发重复。
...
| Beta 当前测试 | `v0.109.0` / `v0.109.1` | `.env` 的 `STS2_ORIGINAL_V1090_REFERENCE_DIR` 或 `STS2_ORIGINAL_V1090_ROOT`（历史名，指向最新 v0.109.1） | — | `original-v0.109.0` | `v0.109.0`（稳定 id，显示 v0.109.x） | — |

v0.109.1 与 v0.109.0 的托管 API/方法 IL 相同，资源变化仅为繁体中文本地化，因此共用同一个 variant；保留 target id 与 ReferenceFlavor 历史名称可避免已有 launch profile 失效。
