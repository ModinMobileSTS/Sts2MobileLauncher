...
- `v0.103.x`、`v0.107.1` / `v0.108.0` stable target、共享 `v0.109.x` / `v0.110.x` target、独立 `v0.111.0` public-beta target、`v0.106.1` beta 与 `v0.107.0` beta target 应保持同一套 Android/Mono MOD 初始化不变式：加载任何 MOD 前只在兼容层 shadow registry 中准备原版模型占位，canonical `ModelDb._contentById` 必须保持 PC 的 pre-init 状态；闭合的 vanilla `ModelDb.GetById<T>` / `GetByIdOrNull<T>` 查询可读取 shadow 对象，但 phase 1 前不得发布到 canonical dictionary，以保留 `ModHelper.AddModelToPool` 等内容注册窗口。每个 MOD initializer 期间只允许短暂隐藏“非原版类型命中早期原版占位”的 `ModelDb.Contains(Type)` 结果，避免与原版同名的 MOD 模型因 Android 提前占位误报重复；若 MOD 因早期占位误判 ModelDb 已初始化并调用 `AbstractModel.InitId()`，兼容层只在 `ModelIdSerializationCache.Init()` 完成前跳过该次早调用，后续正常 `ModelDb.InitIds()` 会统一设置排序 ID，不得提前动态分配 net ID；phase 1 发布 shadow 后，再等所有 MOD Harmony patch 应用完毕按最终 ID 注册 MOD 模型占位。`port-mod/tools/test-modeldb-shadow-placeholder.sh` 必须覆盖 canonical 数量在 MOD 初始化窗口不变、两种 vanilla lookup 可读 shadow、phase 1 才发布。v0.109.0/v0.109.1 托管 API 与方法 IL 相同，`ModelDb.Init(Type[]? injectedModelTypes = null)` 正常 null 路径继续走 two-phase 初始化，显式测试注入集合保留原版行为。用户 MOD 在 initializer 中通过 direct `PatchProcessor.Patch()` 或 `Harmony.PatchAll()` patch STS2 G…
