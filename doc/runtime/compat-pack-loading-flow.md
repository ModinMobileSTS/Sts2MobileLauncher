...
3. BaseLib/RitsuLib/ModelDb/UnlockState 兼容。
   ...
   - `ModelDbInitPatch` 把 `ModelDb.Init` 替换为干净的 two-phase，并分两层处理占位：
     - **早期原版 shadow 占位**：`ModLoaderPatches` 在加载任何 MOD 之前枚举 `AbstractModelSubtypes.All`（仅原版），但只把未初始化对象放入兼容层 shadow registry，不写入原版 `ModelDb._contentById`。Android/Mono 下 MOD initializer 的 Harmony patch 仍可能提前触发 `UnlockState..cctor`、`BowlbugsNormal..cctor` 或其他原版模型读取；兼容层为每个已知 vanilla 类型 patch 闭合的 `ModelDb.GetById<T>` / `GetByIdOrNull<T>`，使这些读取得到 shadow 对象，同时让 canonical ModelDb 继续保持 PC 的 pre-init 状态，避免 `ModHelper.AddModelToPool` 被错误判定为 too late。
   ...
   - 真正的模型构造仍保留在 `OneTimeInitialization.ExecuteEssential -> ModelDb.Init`；用户 MOD 的 `ModelDb.Init` prefix/postfix 仍会执行（prefix `Priority.Last`，跑完后返回 false 跳过原版 one-pass body，postfix 照常运行）；构造前后会清理早期占位枚举产生的 `ModelDb` / 模型实例派生缓存。`port-mod/tools/test-modeldb-shadow-placeholder.sh` 的合成回归验证了 MOD 初始化窗口内 canonical 内容数量不变、两种 vanilla generic lookup 能读取 shadow、phase 1 才发布到 canonical。
