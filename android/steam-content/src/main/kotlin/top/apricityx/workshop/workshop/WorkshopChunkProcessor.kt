...
internal object ChunkProcessor {
    ...
    private fun decompressVzstd(rawChunk: ByteArray): ByteArray {
        ...
        val written = ZstdBridgeInvoker.decompress(destination, compressed)
...
private object ZstdBridgeInvoker {
    fun decompress(destination: ByteArray, compressed: ByteArray): Int {
        try {
            return AndroidZstdBridge.decompress(destination, compressed)
        } catch (error: RuntimeException) {
            throw WorkshopDownloadException("VZstd decompression failed through Android native zstd bridge", error)
        } catch (error: UnsatisfiedLinkError) {
            throw WorkshopDownloadException("Android native zstd bridge is unavailable", error)
        }
    }
}
